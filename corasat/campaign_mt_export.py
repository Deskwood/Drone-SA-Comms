...
def _build_lab_matrix_tex(corasat_root: Path, labs: Sequence[Dict[str, Any]]) -> str:
    ...
    lines.append("\\end{longtable}")
    lines.append("\\noindent\\normalsize\\emph{Note.} Underlined profile identifiers mark the configuration item that differs from the declared parent lab. Rows without an underline denote non-profile steps, such as the initial baseline, the reused reference anchor, or the explicit Optuna stage.")
    lines.append("\\endgroup")
    lines.append("\\end{landscape}")
    lines.append("")
    return "\n".join(lines)
