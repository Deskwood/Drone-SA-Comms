...
def _build_lab_matrix_tex(corasat_root: Path, labs: Sequence[Dict[str, Any]]) -> str:
    ...
    lines.append(
        ...
        "p{0.05\\linewidth} p{0.05\\linewidth} >{\\RaggedRight\\arraybackslash}p{0.10\\linewidth} "
...
def _build_aux_metrics_tex(labs: Sequence[Dict[str, Any]], results_rows: Sequence[Dict[str, str]]) -> str:
    ...
    lines.append("\\renewcommand{\\arraystretch}{0.96}")
    ...
    for lab in runtime_labs:
        ...
        rv_text = f"{rv_true / rv_total:.4f} ({rv_true}/{rv_total})" if rv_total > 0 else "n/a"
        lines.append(
            " & ".join(
                [
                    _latex_escape(lab_id),
