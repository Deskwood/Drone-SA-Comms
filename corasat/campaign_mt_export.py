...
def _format_iso_utc(raw_value: Any) -> str:
...
def _lab_profile_cell(lab: Dict[str, Any], parent: Optional[Dict[str, Any]], field: str) -> str:
    value = str(lab.get(field) or "").strip()
    escaped = _latex_escape(value)
    if isinstance(parent, dict):
        parent_value = str(parent.get(field) or "").strip()
        if value.upper() != parent_value.upper():
            return f"\\underline{{{escaped}}}"
    return escaped
...
def _bool_from_row(row: Dict[str, str], field: str) -> Optional[bool]:
    value = row.get(field)
    if value in (None, ""):
        return None
...
def _format_p_value_display(value: Optional[float]) -> Optional[str]:
...
def _build_lab_matrix_tex(corasat_root: Path, labs: Sequence[Dict[str, Any]]) -> str:
    ...
    lines.append(
        "\\begin{longtable}{p{0.08\\linewidth} p{0.16\\linewidth} "
    ...
    lines.append(
        ...
        "p{0.05\\linewidth} p{0.05\\linewidth} >{\\RaggedRight\\arraybackslash}p{0.43\\linewidth}}"
    )
    lines.append("\\hline")
    lines.append("Lab & Parent & R & P & DS & M & FT & A & C & Hypothesis / role \\\\")
    lines.append("\\hline")
    lines.append("\\endfirsthead")
    lines.append("\\hline")
    lines.append("Lab & Parent & R & P & DS & M & FT & A & C & Hypothesis / role \\\\")
    ...
    for lab in labs:
        ...
        row = [
            ...
            _lab_profile_cell(lab, parent, "rules_id"),
            _lab_profile_cell(lab, parent, "prompt_id"),
            _lab_profile_cell(lab, parent, "drone_support_id"),
            _lab_profile_cell(lab, parent, "model_id"),
            _lab_profile_cell(lab, parent, "fine_tuning_id"),
            _lab_profile_cell(lab, parent, "action_policy_id"),
            _lab_profile_cell(lab, parent, "communication_id"),
    ...
    lines.append("\\end{longtable}")
    lines.append("\\noindent\\normalsize\\emph{Note.} Underlined profile identifiers mark the configuration item that differs from the declared parent lab. Rows without an underline denote non-profile steps, such as the initial baseline, the reused reference anchor, or the explicit Optuna stage. For \\texttt{L4.0}, \\texttt{FT1} overrides the displayed \\texttt{M2} slot at runtime with the Llama-3.1-8B-based \\texttt{corasat-l13-lora} artifact, so the tuple preserves the declared parent relation to \\texttt{L2.3} rather than indicating a same-base-model comparison.")
    lines.append("\\endgroup")
    lines.append("\\end{landscape}")
    lines.append("")
    return "\n".join(lines)
...
def _build_lab_results_tex(labs: Sequence[Dict[str, Any]], lab_rows: Sequence[Dict[str, str]]) -> str:
    ...
    lines.append("\\par\\vspace{2pt}\\noindent\\normalsize\\emph{Note.} For \\texttt{L4.0}, the displayed tuple \\texttt{[R3,P3,DS3,M2,FT1,A3,C0]} preserves the declared parent relation to \\texttt{L2.3}, but \\texttt{FT1} overrides the runtime model with the Llama-3.1-8B-based \\texttt{corasat-l13-lora} artifact.")
    lines.append("\\endgroup")
...
def _build_aux_metrics_tex(labs: Sequence[Dict[str, Any]], results_rows: Sequence[Dict[str, str]]) -> str:
    runtime_labs = [lab for lab in labs if _lab_evaluate_runtime(lab)]
    metrics = ("coverage", "broadcast_rate", "wait_rate", "correct_edge_rate", "avg_turn_duration_s")
    per_lab: Dict[str, Dict[str, List[float]]] = {}
    rendezvous_counts: Dict[str, Dict[str, int]] = {}
    for row in results_rows:
        lab_id = str(row.get("lab_id") or "").strip()
        if not lab_id:
            continue
        ...
        rv_value = _bool_from_row(row, "rendezvous_success")
        if rv_value is not None:
            counts = rendezvous_counts.setdefault(lab_id, {"true": 0, "total": 0})
            counts["total"] += 1
            if rv_value:
                counts["true"] += 1
    ...
    lines.append("\\hline")
    lines.append(
        "Lab & Coverage & Broadcast rate & Rendezvous rate & Wait rate & "
    ...
    for lab in runtime_labs:
        ...
        rv_counts = rendezvous_counts.get(lab_id, {})
        rv_true = int(rv_counts.get("true", 0))
        rv_total = int(rv_counts.get("total", 0))
        rv_text = f"{rv_true / rv_total:.2f} ({rv_true}/{rv_total})" if rv_total > 0 else "n/a"
        lines.append(
            " & ".join(
                [
                    _latex_escape(lab_id),
                    _fmt_mu_sigma(bucket.get("coverage", []), digits=2),
                    _fmt_mu_sigma(bucket.get("broadcast_rate", []), digits=2),
                    _latex_escape(rv_text),
                    _fmt_mu_sigma(bucket.get("wait_rate", []), digits=2),
                    _fmt_mu_sigma(bucket.get("correct_edge_rate", []), digits=2),
                    _fmt_mu_sigma(bucket.get("avg_turn_duration_s", []), digits=2),
                ]
            )
            + " \\\\"
        )
    ...
    lines.append("\\end{tabular}")
    lines.append("\\par")
    lines.append("\\captionsetup{hypcap=false,skip=2pt}")
    ...
    for prev_lab, curr_lab in pairs:
        ...
        if diffs:
            ...
            if p_value is not None and not math.isfinite(float(p_value)):
                p_value = None
            p_display = _format_p_value_display(p_value)
            p_text = test_name if p_display is None else f"{test_name} ({p_display})"
            ci_text = "n/a" if ci_low is None or ci_high is None else f"[{ci_low:+.2f}, {ci_high:+.2f}]"
            lines.append(
                f"{prev_lab} $\\rightarrow$ {curr_lab} & {mean_diff:+.2f} & {ci_text} & {cohen:+.2f} & {p_text} \\\\"
            )
        else:
    ...
    *,
    ...
    if sort_by_metric:
        labels = sorted(
            labels,
            key=lambda lab_id: (
                statistics.median(grouped[lab_id]),
                statistics.mean(grouped[lab_id]),
                _id_sort_key(lab_id),
            ),
            reverse=descending,
        )
...
def _plot_box(
    ...
    if plt is None:
        return None
    labels, values = _metric_series_by_lab(
        labs,
        rows,
        field,
        sort_by_metric=True,
...
def _plot_turn_duration_groups(
    ...
    group_specs = [
        ...
        ("L0.4-L2.1", {"L0.4", "L0.5", "L0.6", "L0.7", "L0.8", "L0.9", "L1.0", "L1.1", "L1.2", "L1.3", "L1.4", "L2.0", "L2.1"}),
        ("L2.2 onward", None),
    ]
...
def _build_campaign_overview_tex(campaign_name: str, manifest_path: Path, results_path: Path, lab_results_path: Path, labs: Sequence[Dict[str, Any]], seed_spec: Any) -> str:
    ...
    finished_utc = _format_iso_utc(manifest.get("finished_at"))
    if started_utc == "n/a" and finished_utc == "n/a":
        run_interval_utc = "n/a"
    else:
        run_interval_utc = f"{started_utc} to {finished_utc}"
    lines.append("% Auto-generated by campaign_mt_export.py")
    lines.append("\\begin{table}[t]")
    lines.append("\\centering")
    lines.append(
...
def export_campaign_artifacts(
    *,
    ...
    score_fig = _plot_box(
        ...
        "norm_score",
        "Normalized score",
        f"{campaign_name}: normalized score",
        fig_generated / "lab_score_distributions.png",
    ...
    bcast_fig = _plot_box(
        ...
        "broadcast_rate",
        "Broadcast rate",
        f"{campaign_name}: broadcast rate",
        fig_generated / "lab_broadcast_rate.png",
        sort_descending=True,
    )
    if bcast_fig:
    ...
    wait_fig = _plot_box(
        ...
        "wait_rate",
        "Wait rate",
        f"{campaign_name}: wait rate",
        fig_generated / "lab_wait_rate.png",
