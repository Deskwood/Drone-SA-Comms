...
- `main.py`: Single entry point. Runs either one runtime config or full `campaign.labs` from `config.json`.
- `config.json`: Master config. Contains global simulation settings, campaign settings, and full L0-L14 lab list.
...
## Entry Commands
- Full campaign: `python main.py --config config.json`
